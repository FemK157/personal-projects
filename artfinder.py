...
def artist(timePeriod, firstLetter, pieceLetter):
    ...
    for artist in data["facets"][0]["facets"]:
        if artist["key"][0] == firstLetter.upper():
...
def displayInfo(number):
    ...
    artist = data['artObject']['principalMakers'][0]['name']
    ...
    st.markdown("<p style='text-align: center; font-size: 20px; color: black'>Primary Artist: {}</p>".format(artist), unsafe_allow_html=True)
...
if firstLetter != "" and pieceLetter != "":
    ...
    try:
        artPiece = st.sidebar.radio("Choose an Art Piece", artpieceTitles)
        index = artpieceTitles.index(artPiece)
        number = artpieceNums[index]
        loadingGIF.empty()
        displayInfo(number)
    except:
        loadingGIF.empty()
